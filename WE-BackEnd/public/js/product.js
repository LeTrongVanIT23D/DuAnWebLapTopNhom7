    ...
    $("#sample_data").DataTable({
      ...
      columns: [
...

        {
          data: "price",
          render: function (data) {
            // Format tiền Việt Nam cho đẹp (tùy chọn)
            return '<div class= "my-3">' + new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(data) + "</div>";
          },
        },
        {
          // --- ĐÃ SỬA: Đổi từ inventory thành quantity ---
          data: "quantity",
          render: function (data) {
            // Kiểm tra nếu undefined hoặc null thì hiện 0
            return '<div class= "my-3">' + (data !== undefined && data !== null ? data : 0) + "</div>";
          },
        },
        {
          data: null,
...
$("#add_data").click(function () {
  ...
  // Reset giá trị quantity về 0 hoặc rỗng khi thêm mới
  $("#quantity").val(""); 
...
$(document).on("click", ".edit", function () {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      // --- ĐÃ SỬA: Thêm dòng này để điền số lượng khi sửa ---
